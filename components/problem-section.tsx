...
export default function ProblemSection() {
  ...
  const challenges = [
    ...
    },
    {
      ...
      icon: <HeartPulse className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />,
    },
    {
    ...
    },
    {
      ...
      icon: <Search className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />,
    },
  ]
        ...
        >
          <motion.div className="space-y-4 sm:space-y-6" variants={containerVariants}>
            ...
            <motion.div className="space-y-3 sm:space-y-4" variants={containerVariants}>
              ...
              <motion.div
                className="flex items-center gap-2 p-3 sm:p-4 bg-destructive/10 rounded-lg border border-destructive/20 transform-gpu hover:shadow-md transition-all duration-300 mt-3 sm:mt-4"
                variants={itemVariants}
              >
          ...
          >
            <div className="p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6">Você se identifica com estes desafios?</h3>
              ...
              <ul className="space-y-3 sm:space-y-5">
                {challenges.map((challenge, index) => (
                  ...
                  >
                    <div className="rounded-full bg-primary/10 p-1 sm:p-1.5 mt-0.5 group-hover:bg-primary/20 transition-colors duration-300 flex-shrink-0">
                      {challenge.icon}
                    </div>
                    <span className="group-hover:text-primary/90 transition-colors duration-300 text-sm sm:text-base">
                      {challenge.text}
                    </span>
                  </motion.li>
                ))}
              ...
              <motion.div
                className="mt-6 sm:mt-8 p-4 sm:p-5 bg-gradient-to-br from-primary/5 to-primary/20 rounded-lg"
                initial={{ opacity: 0, y: 20 }}
                animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
                transition={{ duration: 0.7, delay: 0.8 }}
              >
                <div className="flex items-start gap-2 sm:gap-3">
                  <div className="rounded-full bg-primary/20 p-1 sm:p-1.5 mt-1 flex-shrink-0">
                    <HelpCircle className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />
                  </div>
                  <div>
                    <p className="font-medium text-base sm:text-lg mb-1 sm:mb-2">
                      E se o real desafio estiver em outro lugar?
                    </p>
